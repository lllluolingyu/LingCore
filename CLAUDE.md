...
uv run pytest -q              # run the full suite (337 tests, <10s when tiktoken is cached)
...
- `lingcore/media.py` — attachment construction helpers for tools/frontends (`attachment_from_path`/`attachment_from_wire`); converts validation failures to `ToolError`.
- `lingcore/modality.py` — `MediaAdapter` + `extract_pdf_markdown`: text fallbacks for attachment kinds the model doesn't accept (`llm.modalities` registration + `media_fallback` config). Takes a duck-typed vision client; never imports `llm.py`/the SDK. pymupdf is loaded lazily (optional `lingcore[pdf]` extra).
...
15. **Attachment modalities are config-registered; degradation never crashes the loop.** `llm.modalities` declares which attachment kinds the model natively accepts (default: both `image` and `file` — today's behavior). When a kind is missing, `MediaAdapter.prepare` computes the attachment's `fallback_text` **once at ingest** (PDF → extracted markdown, image → a description from the `media_fallback.image` vision model) and **never raises** — failures, including a missing pymupdf, become diagnostic notes the model can read. Originals are never stripped: sessions/LingChat keep full media, so raising `modalities` later restores native delivery, and `model_copy` keeps validators out of the hot path. The native-vs-text decision happens per render in `Message.to_openai(attachment_modalities=...)` (the `LLMClient` passes its set), and when **no native part remains the content collapses to a plain string** — text-only servers may reject a parts array. The describe call goes through the same duck-typed `stream` seam as the main model and always renders natively (config validation rejects a vision model whose own `modalities` exclude `image`).
